...
constexpr int PIN_LED[3]{20, 10, 8};
constexpr int POT_READ_PIN[3]{0, 1, 4};
...
void dimRGB_loop() {
    ...
    for (int i = 0; i < 3; i++) {                   // for loop to read from each potentiometer and write to its respective LED output Pin
        potValue[i] = analogRead(POT_READ_PIN[i]);    // store the analog value read from the potentiometer between 0 - 4095
        brightness[i] = potValue[i] / 16;           // divide the stored value by 16 to get it within a 0-255 for PWM
        analogWrite(PIN_LED[i], brightness[i]);      // set the LED brightness to the calculated number
    }
